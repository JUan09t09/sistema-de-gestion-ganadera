...
function construirNavegacion() {
  ...
  navEl.innerHTML = perfil.modulos.map(function(mod) {
  ...
  // Mostrar/ocultar páginas según perfil
  document.querySelectorAll('.page').forEach(function(pg) {
    const nombre = pg.id.replace('page-','');
    pg.style.display = perfil.modulos.indexOf(nombre) !== -1 ? '' : 'none';
  });
}
...
function guardarRepro() {
  ...
  if (fechaParto) alert('✅ Parto estimado: '+fmt(fechaParto));
}
...
function guardarPesaje() {
  ...
  if (!animalId||!fecha||!kg){ alert('⚠️ Completa todos los campos'); return; }
...
function construirNavegacionConRol() {
  ...
  // Gestión de usuarios solo para propietario
  if (perms.gestionUsuarios) {
...
// Intercepta llamadas a guardarXxx y verifica permiso
const _guardarAnimalOrig = guardarAnimal;
function guardarAnimal() {
  if (!puedo('editarAnimales')) { alert('⚠️ No tienes permiso para editar animales'); return; }
  _guardarAnimalOrig();
}

const _guardarLecheOrig = guardarLeche;
function guardarLeche() {
  if (!puedo('editarLeche')) { alert('⚠️ No tienes permiso para registrar leche'); return; }
  _guardarLecheOrig();
}

const _guardarSaludOrig = guardarSalud;
function guardarSalud() {
  if (!puedo('editarSalud')) { alert('⚠️ No tienes permiso para registrar eventos médicos'); return; }
  _guardarSaludOrig();
}

const _guardarPesajeOrig = guardarPesaje;
function guardarPesaje() {
  if (!puedo('editarCarne')) { alert('⚠️ No tienes permiso para registrar pesajes'); return; }
  _guardarPesajeOrig();
}

const _guardarReproOrig = guardarRepro;
function guardarRepro() {
  if (!puedo('editarRepro')) { alert('⚠️ No tienes permiso para registrar eventos reproductivos'); return; }
  _guardarReproOrig();
}

const _guardarGastoOrig = guardarGasto;
function guardarGasto() {
  if (!puedo('editarFinanzas')) { alert('⚠️ No tienes permiso para registrar gastos'); return; }
  _guardarGastoOrig();
}

const _guardarVentaOrig = guardarVenta;
function guardarVenta() {
  if (!puedo('editarFinanzas')) { alert('⚠️ No tienes permiso para registrar ingresos'); return; }
  _guardarVentaOrig();
}

const _guardarPotreroOrig = guardarPotrero;
function guardarPotrero() {
  if (!puedo('editarPotreros')) { alert('⚠️ No tienes permiso para gestionar potreros'); return; }
  _guardarPotreroOrig();
}

const _guardarConfigOrig = guardarConfig;
function guardarConfig() {
  if (!puedo('editarConfig')) { alert('⚠️ No tienes permiso para cambiar la configuración'); return; }
  _guardarConfigOrig();
}

// ---- Hook en mostrarAppTrabajador para mostrar banner ----
const _origMostrarAppTrab = mostrarAppTrabajador;
function mostrarAppTrabajador() {
  _origMostrarAppTrab();
  setTimeout(function() {
    mostrarBannerTrabajador();
    construirNavManual();
    iniciarNotificaciones();
  }, 500);
}
